...
class TestProgressiveStack:
    ...
    def test_setup_enables_tensor(self):
        ...
        tensor_module_path = Path(__file__).parent.parent.parent / "srcs" / "tinytorch"
        assert (
            tensor_module_path.exists()
        ), f"Setup didn't create proper module structure. {tensor_module_path}"
