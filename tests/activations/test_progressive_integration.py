...
class TestModule01StillWorking:
    ...
    def test_setup_environment_stable(self):
        ...
        assert (project_root / "srcs").exists(), "Module structure broken"
        assert (project_root / "srcs" / "tinytorch").exists(), "Module structure broken"
...
class TestModule03ActivationsCore:
    ...
    def test_sigmoid_activation(self):
        ...
        try:
            ...
            ), "Sigmoid activation failed"

            # All outputs should be in (0, 1)
            assert np.all(output.data > 0) and np.all(
                output.data < 1
            ), "Sigmoid range failed"
    ...
    def test_nonlinearity_proof(self):
        ...
        try:
            ...
            assert not np.array_equal(
                linear_output, nonlinear_output
            ), "No nonlinearity detected"
